/*SM4算法实现
SM4是对称结构加解密算法一致，秘钥倒序
加解密：128bit->4*32bit，共32轮	F（Xi，Xi+1,Xi+2,Xi+3,rki） ，最后一轮交换位置
	F函数：异或-S盒-循环移位-异或
秘钥扩展：128bit->4*32bit，初始化-轮变换
	初始化：与FK异或
	轮变换：异或-S盒-循环移位-异或
*/

#include<stdio.h>
#include<memory.h>

typedef unsigned char unit8;//定义8位的存储单元
typedef unsigned int unit32;//定义32位的存储单元

const unit8 S[256]={//S盒

};

int Swap(unit32 data[4])//4个32位分组位置反序
{
	unit32 temp = 0;
	temp = data[0];
	data[0] = data[3];
	data[3] = temp;
	temp = data[2];
	data[2] = data[1];
	data[1] = temp;
	return 0;
}

int SBox(unit32 &data)//S盒变换
{//32位的数组，每8位一组

	data = S[data >> 24 & 0xff] << 24 | S[data >> 16 & 0xff] << 16 | S[data >> 8 & 0xff] << 8 | S[data & 0xff];
	return 0;
}

int RotL(unit32 data,int n)//data循环左移n位
{
	data = (data << n)|(data>>(32-n));
	return data;
}

int LS_1(unit32 &data)//data与自身循环左移2,10,18,24进行异或
{
	data = data ^ RotL(data, 2) ^ RotL(data, 10) ^ RotL(data, 18) ^ RotL(data,24);
	return 0;
}

int Encrypto(unit8 input[16],unit8 output[16],unit32 rk[32])//输入输出，128bit，秘钥32轮子秘钥
{
	//输入输出都是16个8bit，要转换成4个32bit；
	unit32 p[4],temp;
	memcpy(p, input, 16);//拷贝顺序反序，大小端问题
	//进行32轮的迭代
	for (int i = 0; i < 32;i++)
	{
		temp=p[(i+3)%4] ^ p[(i+2)%4] ^ p[(1+i)%4] ^ rk[i];//异或
		SBox(temp);
		LS_1(temp);
		p[i%4] = p[i%4] ^ temp;
	}
	Swap(p);
	memcpy(output, p, 16);
	return 0;
}

int Decrypto(unit8 input[16],unit8 output[16],unit32 rk[32])
{
	unit32 temp;
	for(int i=0;i<16;i++)
	{
		temp=rk[i];
		rk[i]=rk[31-i];
		rk[31-i]=temp;
	}
	Encrypto(input,output,rk);
	return 0;
}

int main()
{
	unit8 input[16]={0x11,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98 ,0x76,0x54,0x32,0x10};
	unit8 output[16]={0};
	unit32 rk[32]={0};//32轮秘钥
	for(int i=0;i<32;i++){
		rk[i]=0x20221109;
	}

	printf("-------------SM4算法实现-----------------\n");
	printf("加密前的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",input[i]);
	}

	Encrypto(input,output,rk);//调用加密函数
	printf("\n加密后的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",output[i]);
	}

	unit8 temp[16]={0};
	Decrypto(output,temp,rk);//调用解密函数
	printf("\n解密后的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",temp[i]);
	}
	printf("\n");
	return 0;
}

