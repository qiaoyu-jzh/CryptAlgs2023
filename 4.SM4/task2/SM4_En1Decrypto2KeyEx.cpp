/*SM4算法实现
SM4是对称结构加解密算法一致，秘钥倒序
加解密：128bit->4*32bit，共32轮	F（Xi，Xi+1,Xi+2,Xi+3,rki） ，最后一轮交换位置
	F函数：异或-S盒-循环移位-异或
秘钥扩展：128bit->4*32bit，初始化-轮变换
	初始化：与FK异或
	轮变换：异或-S盒-循环移位-异或
*/

#include<stdio.h>
#include<memory.h>

typedef unsigned char unit8;//定义8位的存储单元
typedef unsigned int unit32;//定义32位的存储单元

const unit8 S[256]={

};

const unit32 ck[32]=
{
	0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
	0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
	0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
	0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
	0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
	0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
	0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
	0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
};

int SBox(unit32 &data)
{
	data=S[data>>24&0xFF]<<24|
	S[data>>16&0xFF]<<16|
	S[data>>8&0xFF]<<8|
	S[data&0xFF];
	return 0;
}

unit32 RotL(unit32 data,int n)//data循环左移n位
{
	data=(data<<n)|(data>>(32-n));
	return data;
}

int LS_1(unit32 &data)//data与自身循环左移2,10,18,24进行异或
{
	data=data^RotL(data,2)^RotL(data,10)^RotL(data,18)^RotL(data,24);
	return 0;
}

int LS_2(unit32 &data)//data与自身循环左移13,23进行异或
{
	data=data^RotL(data,13)^RotL(data,23);
	return 0;
}

int KeyExt(unit8 key[16],unit32 rk[32])//输入128位主秘钥，输出32轮子秘钥
{
	unit32 k[4],temp;
	memcpy(k,key,16);
	k[0]^=0xa3b1bac6;
	k[1]^=0x56aa3350;
	k[2]^=0x677d9197;
	k[3]^=0xb27022dc;//初始化
	for(int i=0;i<32;i++)
	{
		temp=k[(i+1)%4]^k[(i+2)%4]^k[(i+3)%4]^ck[i];
		SBox(temp);//过S盒
		LS_2(temp);	//循环移位异或
		rk[i]=k[i%4]^temp;////////少了K的迭代赋值
	}
	return 0;
}

int Swap(unit32 data[4])//
{
	unit32 temp;
	temp=data[0];
	data[0]=data[3];
	data[3]=temp;
	temp=data[1];
	data[1]=data[2];
	data[2]=temp;
	return 0;
}

int Encrypto(unit8 input[16],unit8 output[16],unit32 rk[32])//输入输出，128bit，秘钥32轮子秘钥
{
	unit32 p[4],temp;
	memcpy(p,input,16);//16*8bit转换成4*32bit

	for (int i=0;i<32;i++)
	{
		temp=p[(i+1)%4]^p[(i+2)%4]^p[(i+3)%4]^rk[i];//异或
		SBox(temp);//过S盒
		LS_1(temp);	//循环移位异或
		p[i%4]^=temp;
	}
	Swap(p);
	memcpy(output,p,16);
	return 0;
}

int Decrypto(unit8 input[16],unit8 output[16],unit32 rk[32])
{
	unit32 temp;
	for(int i=0;i<16;i++)
	{
		temp=rk[i];
		rk[i]=rk[31-i];
		rk[31-i]=temp;
	}
	Encrypto(input,output,rk);
	return 0;
}

int main()
{
	unit8 input[16]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
	unit8 output[16]={0};
	unit8 key[16]={0x01,0x23,0x45,0x67,0x89,0xab,0xcd,0xef,0xfe,0xdc,0xba,0x98,0x76,0x54,0x32,0x10};
	unit32 rk[32]={0};
	unit8 temp[16]={0};

	printf("-------------SM4算法实现-----------------\n");
	printf("加密前的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",input[i]);
	}
	printf("\n");

	KeyExt(key,rk);
	Encrypto(input,output,rk);

	printf("加密后的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",output[i]);
	}
	printf("\n");

	Decrypto(output,temp,rk);
	printf("解密后的字符串是：");
	for(int i=0;i<16;i++)
	{
		printf("%x ",temp[i]);
	}
	printf("\n");

	return 0;
}

